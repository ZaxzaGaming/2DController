...
public class PlayerData : ScriptableObject
{
    ...
    [Range(0.5f, 2)] public float doubleJumpDivider;
    ...
    public Vector2 wallClimbJumpForce;//The actual force (this time set by us) applied to the player when wall jumping.
    ...
    public float glideAcceleration; //The speed at which our player accelerates to max speed, can be set to runMaxSpeed for instant acceleration down to 0 for none at all
    [HideInInspector] public float glideAccelAmount; //The actual force (multiplied with speedDiff) applied to the player.
    public float glideDecceleration; //The speed at which our player decelerates from their current speed, can be set to runMaxSpeed for instant deceleration down to 0 for none at all
    [HideInInspector] public float glideDeccelAmount;
    ...
    [Tooltip("Freeze game for split second")]
    ...
    private void OnValidate()
    {
        ...
        doubleJumpForce = jumpForce / doubleJumpDivider;
        tripleJumpForce = jumpForce / tripleJumpDivider;

        #region Variable Ranges
        runAcceleration = Mathf.Clamp(runAcceleration, 0.01f, runMaxSpeed);
        runDecceleration = Mathf.Clamp(runDecceleration, 0.01f, runMaxSpeed);

        glideAcceleration = Mathf.Clamp(glideAcceleration, 0.01f, glideSpeed);
        glideDecceleration = Mathf.Clamp(glideDecceleration, 0.01f, glideSpeed);
        #endregion
    }
}